/**
 * Hardcoded V11 design-demo interactions (ported from design-reference/v11/script.js).
 * Intentionally DOM-driven for visual parity; swap for React state + contractService later.
 */
export function initDesignDemo() {
...
function showToast(message) {
  clearTrackedTimeout(toastTimer);
...
function openPromiseModal() {
  ...
  trackTimeout(() => document.querySelector("#closePromiseModal")?.focus(), 60);
}
...
document.querySelectorAll(".scene-option").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (isCustomScene) trackTimeout(() => customSceneName.select(), 60);
  });
});
...
document.querySelector("#viewPromiseCard").addEventListener("click", () => {
  closePromiseModal();
  trackTimeout(() => openConfirmFlow(document.querySelector("#openConfirmFlow")), 180);
});
...
function openConfirmFlow(origin) {
  ...
  document.body.classList.add("modal-open");
  trackTimeout(() => document.querySelector("#closeConfirmFlow")?.focus(), 60);
}
...
function openExitFlow(origin) {
  ...
  document.body.classList.add("modal-open");
  trackTimeout(() => document.querySelector("#closeExitFlow")?.focus(), 60);
}
...
function openRescueFlow(ticket, origin) {
  ...
  document.body.classList.add("modal-open");
  trackTimeout(() => document.querySelector("#closeRescueFlow")?.focus(), 60);
}
...
if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
  ...
  return function cleanupDesignDemo() {
