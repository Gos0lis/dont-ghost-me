...
export function CreateBountyPage() {
  ...
  const availablePool = useMemo(
    () => Math.max(0, (project?.rescuePool ?? 0) - (project?.reservedBounty ?? 0)),
    [project?.rescuePool, project?.reservedBounty],
  )
  const remaining = useMemo(() => availablePool - input.reward, [availablePool, input.reward])

  useEffect(() => {
    setInput((current) => current.reward > availablePool ? { ...current, reward: availablePool } : current)
  }, [availablePool])
      ...
      <div className="bounty-create-layout">
          ...
          <div className="form-section">
            ...
            <div className="form-grid single">
              ...
              <div className="form-grid">
                ...
                <FormField label="奖励金额（MON）" hint={`最多可发布 ${availablePool} MON`}><TextInput type="number" max={availablePool} value={input.reward} onChange={(event) => setInput({ ...input, reward: Math.min(availablePool, Math.max(0, Number(event.target.value))) })} /></FormField>
              </div>
        ...
        <aside className="bounty-funds">
          ...
          <div className="bounty-balance-stack">
            <BalanceCard label="当前可用救场池" value={availablePool} unit="MON" accent="orange" />
          ...
          <PrimaryButton disabled={remaining < 0 || input.reward <= 0} icon={<Banknote size={17} />} onClick={() => setConfirmOpen(true)}>发布 {input.reward} MON 悬赏</PrimaryButton>
        </aside>
      </div>
      <ConfirmDialog
        open={confirmOpen}
