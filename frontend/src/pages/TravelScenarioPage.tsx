...
const rescueTasks = [
  ...
  },
  {
    ...
    reward: 40,
    title: '转卖闲置门票',
    description: '将 Momo 名下无法退订的景点门票合规转让，减少团队损失。',
    skills: ['票务', '闲置交易'],
...
export function TravelScenarioPage() {
  ...
  const momo = project?.members.find((member) => member.id === 'momo')
  const hasQuit = momo?.status === 'quit'
    ...
    <div className="travel-demo-page">
      <div className="page-shell">
          ...
          <div>
            ...
            <div className="travel-hero-actions">
              {!hasQuit ? (
                ...
                <Link to="/bounties"><PrimaryButton>前往悬赏大厅 <ArrowRight size={16} /></PrimaryButton></Link>
              )}
        ...
        <section className="travel-story">
          ...
          <div className={`travel-step ${hasQuit ? 'danger' : ''}`}><span>02</span><Plane size={20} /><div><strong>Momo 临时退出</strong><p>{hasQuit ? '100 MON 已进入补救悬赏池。' : '点击上方按钮模拟链上退出交易。'}</p></div></div>
                ...
                return (
                  <article className={bounty ? 'published' : ''} key={task.title}>
                    ...
                    <div className="travel-task-status">
                      {bounty ? <><CheckCircle2 size={15} />已发布到救场大厅</> : <>等待发布</>}
                    </div>
      ...
      <ConfirmDialog
        open={quitOpen}
        title="确认 Momo 临时退出旅行？"
        description="交易成功后，Momo 锁定的 100 MON 不会平分给朋友，而会进入旅行补救悬赏池。"
        confirmLabel="确认退出并转入补救池"
        danger
        loading={pendingMethod === 'quitProject'}
        details={<><span>合约方法</span><strong>quitProject("five-friends-trip", "momo")</strong><span>转入补救池</span><strong>100 MON</strong></>}
        onClose={() => setQuitOpen(false)}
        onConfirm={() => void quitProject(TRAVEL_PROJECT_ID, 'momo').then(() => setQuitOpen(false)).catch(() => undefined)}
      />
      <ConfirmDialog
        open={publishOpen}
        title="发布三项旅行补救悬赏？"
        description="系统将依次执行三笔模拟链上交易，把 Momo 的 100 MON 分配为 40、40、20 MON 三个可领取任务。"
